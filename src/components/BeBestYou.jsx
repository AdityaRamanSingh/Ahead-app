import React from "react";

const BeBestYou = () => {
  return (
    <section className="my-[5rem] mb-5 grid grid-cols-3 px-20 gap-10">
      <div className="text-4xl font-bold p-4 px-15 ml-20">
        Be the best You with EQ
      </div>
      <div className="text-xl p-4 px-30 ml-20">
        Not having your own emotions under control might be holding you back.
      </div>
      <div className="text-xl p-4 px-30 mr-20">
        Additionally, not understanding those of others stops you from being
        parent, friend you can be.
      </div>
    </section>
  );
};

export default BeBestYou;